...
The nested class `FileChannel.MapMode` is a logical enum (it is technically not an `enum`, because it was introduced before enums):
```java
...
After this new unsafe addition to the class, which shouldn't be used, `jdk.internal.misc.Unsafe` has 431 members in total.
