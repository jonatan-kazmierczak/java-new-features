...
API allowing to allocate memory outside of heap and to access it in a safe way, included in a new module `jdk.incubator.foreign`.  
It is intended to replace equivalent functionality existing in `Unsafe` class, and to be an alternative to `ByteBuffer` API.
...
Extends `FileChannel` API to allow creation of `MappedByteBuffer` instances operating on non-volatile memory (on the contrary to the existing functionality, operating on RAM - a volatile memory).  
It targets Linux x64.

The abstract class `FileChannel` - an entry point to the API - is shown on the below diagram with its references and usages 
(class diagram generated by [Class Visualizer](https://class-visualizer.net/)):
![diagram](diagrams/file-channel.png)

The nested class `java.nio.channels.FileChannel.MapMode` is a logical enum (it is technically not an `enum`, because it was introduced before enums):
```java
...
```
(please note a private constructor of the class).

Now, the new module `jdk.nio.mapmode` is introduced. It contains only one class `jdk.nio.mapmode.ExtendedMapMode` "extending" `FileChannel.MapMode`.  
...
public class ExtendedMapMode {
  ...
  static {
    READ_ONLY_SYNC = jdk.internal.misc.ExtendedMapMode.READ_ONLY_SYNC;
    READ_WRITE_SYNC = jdk.internal.misc.ExtendedMapMode.READ_WRITE_SYNC;
  };
}
```
Isn't it an unusual way of "extending" classes?

So, there is another introduced class in the core package `java.base`: `jdk.internal.misc.ExtendedMapMode`. 
...
```
What is this class for? Well, it's job is... creating 2 new instances of `FileChannel.MapMode` - 
it uses `MethodHandle` to invoke a private constructor of `FileChannel.MapMode`.

There is some other addition as well - a new method in a famous class `jdk.internal.misc.Unsafe`:  
`writebackMemory( long address, long length )`.  
After this new addition to the class, which shouldn't be used, `jdk.internal.misc.Unsafe` has 431 members in total.
